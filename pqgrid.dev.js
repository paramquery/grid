/**
 * ParamQuery Grid a.k.a. pqGrid v1.0.4
 * 
...
(function ($) {
	...
	_p.cleanNotUsed=function(){	
		var selectedRows=this.selectedRows;	
		for (var i = 0; i < selectedRows.length; i++) {
			var selRow=selectedRows[i],
				rowIndx=selRow.rowIndx;
			if (selRow.$tr) {
				selRow.$tr.removeClass("pq-row-select");
				selRow.$tr=null;
			}
			this._removeFromData(rowIndx);			
		}
	}
	_p.buildnotused=function(){
		var selectedRows=this.selectedRows;
		var offset=this.that.getRowIndxOffset();
		for(var i=0;i< selectedRows.length;i++){
			var selRow=selectedRows[i];
			var rowIndxPage=selRow.rowIndx-offset;
			selRow.$tr=this._boundRow({rowIndxPage:rowIndxPage});
		}	
	}
	_p.buildIndicesFromData=function(){
		this.selectedRows=[];
	...
	_pC.cleanNotUsed=function(){
		var selectedCells=this.selectedCells;
		for (var i = 0; i < selectedCells.length; i++) {
			var selCell=selectedCells[i];
			if (selCell.$td) {
				selCell.$td.removeClass("pq-cell-select");
				selCell.$td=null;
			}
		}
	}
	_pC.buildIndicesFromData=function(){
		this.selectedCells=[];
		var data=this.options.dataModel.data;
		for(var i=0,len=data.length;i<len;i++){
			var row=data[i],
	...
	_pC.buildNotUsed=function(){
		var selectedCells=this.selectedCells;
		var offset=this.that.getRowIndxOffset();		
		for(var i=0;i< selectedCells.length;i++){
			var selCell=selectedCells[i];
			var rowIndxPage=selCell.rowIndx-offset;
			selCell.$td=this._boundCell({rowIndxPage:rowIndxPage,colIndx:selCell.colIndx});
		}	
	}								
	...
	_pC._boundCellNotUsed=function(obj){
		var rowIndxPage=obj.rowIndxPage,
			colIndx=obj.colIndx,
			that=this.that,
			$td=(obj.$td==null)?that.getCell({rowIndxPage:rowIndxPage,colIndx:colIndx}):obj.$td;			
    ...
    fn.keyPressDown = function (evt) {
        ...
        if (that.$td_edit) {
            ...
            if (evt.keyCode == keyCodes.tab) { 
                ...
                evt.preventDefault();
                return false;
			} else if ((evt.keyCode == keyCodes.up)||(evt.keyCode == keyCodes.down)) {
				...
				if (this.options.selectionModel.type == 'row') {
					...
					that._bringCellIntoView({rowIndx:rowIndx, colIndx:colIndx});
				}
				else {
					that._setSelection(null);
					that._setSelection({
				...
				this._editCell( this.getCell( {rowIndx:rowIndx, colIndx:obj.colIndx} ) );
				
                evt.preventDefault();
                return false;
			} else if (evt.keyCode == (colSaveKey ? colSaveKey : this.options.editModel.saveKey)) {
                that.saveEditCell();
                that.quitEditMode(evt);																			
